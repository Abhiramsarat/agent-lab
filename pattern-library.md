...
- **Fix at the source, not the surface.** If a generator (compiler) produces the problem from its inputs, editing outputs is whack-a-mole — correct the inputs and regenerate. Triage each finding: record-level → fix input + recompile; copy-level → targeted edit. (Loop v0.2's planner.)
- **Constrain the repairer to the client's words.** A repairer WILL embellish ("referral basis") unless explicitly forbidden to characterize beyond the source material — and a good evaluator will catch its teammate doing it.
- **Objective findings converge; subjective findings oscillate.** An evaluator-optimizer loop resolves facts fast but can ping-pong on taste (how visible is "downplayed"?). Route judgment-call findings to the human gate ("accept-with-note"); don't let the loop argue aesthetics with itself.
- **Evaluators are stochastic.** Same input → different finding counts across runs. Design gates with severity classes, not a binary "clean."

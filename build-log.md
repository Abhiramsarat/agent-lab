...
## 2026-06-29 (cont.) — LOOP v0.2: fix-at-source. The demo exists.
- Built the PLANNER (`triage.mjs`): triages findings → RECORD ops (correct the compile record, recompile) vs COPYFIX (prose edits). Deterministic executors; client's original inputs never altered; auditor always judges against original truth.
- Star moments: planner proposed `setLeadPractice(banking-psu)` from the client's session note ON ITS OWN (real judgment, not in any finding); the auditor CAUGHT THE REPAIRER'S OWN embellishment ("referral basis") — the system polices itself.
- Final run: 7 → 6 → 3 → 1; the sole residual is a subjective visibility judgment (downplay-vs-delete taste), correctly escalated to humans. All factual/structural findings resolved.
- Production insights harvested: (1) audit needs structured intake in its inputs; (2) the seeded services catalog injects dozens of unconfirmed specialisms (SOC 2, ISO 27001…) = invention risk at scale; (3) the auditor is non-deterministic run-to-run → production needs severity classes (must-fix vs judgment) rather than a binary clean gate.
